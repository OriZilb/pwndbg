...
RUN_FILE_TO_USE_PWNDBG_COMMSNDS = [f"'file {binary_reference}'", "'b main'", "r",]
...
def build_lldb_command(commands, run_file_to_use_pwndbg_commands):
    ...
    command = [os.environ["DEBUGGER_COMMAND"], "--silent", "--commands"] + run_file_commands + \
             [f"'{command}'" for command in commands] + ["c", "quit;", os.environ["LLDB_FLAGS"]]
    return " ".join(command)
...
def run_lldb_command(commands, run_file_to_use_pwndbg_commands):
    return subprocess.run(
            ...
            text=True, shell=True, capture_output=True, stdin=subprocess.DEVNULL
    )
